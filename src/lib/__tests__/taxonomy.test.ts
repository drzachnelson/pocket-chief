...
describe("taxonomySections", () => {
  it("returns every top-level category in taxonomy order", () => {
    ...
    expect(sections.map((section) => section.node.id)).toEqual(["alimentary", "breast", "general-abdomen", "hernia", "arterial-disease", "critical-care", "esophagus", "pediatric", "trauma"]);
  });
