...
describe("seeded content contract", () => {
  ...
  it("holds the launch topic to the digest pinned in the Supabase migration", () => {
    const sql = readFileSync("supabase/migrations/202608130001_release_hardening.sql", "utf8");
    const pinned = sql.match(/'([0-9a-f]{64})' then raise exception 'Launch topic content did not match/)?.[1];
    const digest = createHash("sha256").update(JSON.stringify(choledoBlocks), "utf8").digest("hex");
    expect(digest, "choledoBlocks must serialize byte-identically or ensure_launch_topic rejects it").toBe(pinned);
  });
});
