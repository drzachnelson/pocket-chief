...
export function AddNoteForm({ taxonomy }: { taxonomy: TaxonomyNode[] }) {
  ...
  async function submit(event: React.FormEvent, mode: "ai" | "notes_only" = "ai") {
    ...
    setBusy(true);
    try {
      const response = await fetch("/api/topics/drafts", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title, rawNotes: notes, imageIds: [], sourceMetadata: [{ title: `${title} owner-supplied notes`, kind: "user_notes", citation: sourceDetails || `${title} personal study packet`, details: sourceDetails || undefined }], scoreNodeId, tags: tags.split(",").map((tag) => tag.trim()).filter(Boolean), mode }) });
  ...
  return (
    <form className="form-card" onSubmit={(event) => submit(event, "ai")}>
      ...
      <div className="field"><label htmlFor="personal-tags">Personal tags</label><input id="personal-tags" value={tags} onChange={(event) => setTags(event.target.value)} placeholder="absite, biliary, operative" /><small>Separate tags with commas. They become searchable after approval.</small></div>
