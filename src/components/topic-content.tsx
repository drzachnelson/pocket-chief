...
function Block({ block, onMakeCard }: { block: TopicBlock; onMakeCard: (text: string, block: TopicBlock) => void }) {
  ...
  if (block.type === "image") return <figure id={block.id} data-block-id={block.id} className="topic-block"><Image src={block.mediaId} alt={block.alt} width={1200} height={630} unoptimized />{block.caption && <figcaption>{block.caption}</figcaption>}</figure>;
  return null;
}
...
function AnkiDialog({ selection, topic, block, onClose }: { selection: string; topic: Topic; block: TopicBlock; onClose: () => void }) {
  ...
  async function draft(): Promise<ClozeDraft> {
    ...
    return { id: crypto.randomUUID(), clozeText: text, additionalContext: `Pocket Chief · ${topic.title} · ${block.heading ?? "Note"}`, sourceBlockIds: [block.id], contextImageRef: contextImageDataUrl(block.heading ?? topic.title, excerpt(block), diagram), tags: ["pocket-chief", ...topic.tags], duplicateHash: await createDuplicateHash(text.replace(/{{c\d+::|}}/g, "")) };
  }
  async function copyTsv() { const value = toAnkiTsv([await draft()]); await navigator.clipboard.writeText(value); setStatus("TSV copied"); }
  async function openMobile() { window.location.href = buildAnkiMobileUrl(await draft(), { deck: "Pocket Chief", noteType: "Cloze", tagPrefix: "pc::" }); }
  async function exportDesktop() {
    const card = await draft();
    const settings = { deck: "Pocket Chief", noteType: "Cloze", tagPrefix: "pc::", fieldMap: { text: "Text", extra: "Extra" } };
...
export function TopicContent({ topic, sources }: { topic: Topic; sources: SuppliedSource[] }) {
  ...
  useEffect(() => { recordRecentView(topic).catch(() => undefined); isTopicSaved(topic.id).then(setSaved).catch(() => undefined); }, [topic]);

  function toggleSaved() { const next = !saved; setSaved(next); setTopicSaved(topic, next).catch(() => setSaved(!next)); }
  ...
  return (
    <>
      <div className="topic-actions-row"><button className={`button secondary small ${saved ? "saved" : ""}`} onClick={toggleSaved}><BookmarkSimple size={14} weight={saved ? "fill" : "regular"} />{saved ? "Saved offline" : "Save"}</button><span className="status-pill"><span className="status-dot" />Version {version.versionNumber} reviewed</span></div>
