...
**Four behaviour changes the owner should expect, called out here so nobody rediscovers them as bugs:**
...
3. **The backup ZIP is gone.** Settings offered a "Download backup" link to `/api/backup`, producing a ZIP of approved topic Markdown and JSON, source metadata, Anki drafts, and a media manifest. Nothing replaces it, and nothing needs to: the topics it exported now live in this git repository, which is a better backup than a ZIP, and the Anki drafts it also carried no longer exist. Worth knowing it is deliberate — it never contained bookmarks or reading history, so its removal loses nothing that the next item does not already cover.
